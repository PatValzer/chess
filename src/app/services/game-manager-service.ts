...
export class GameManagerService {
  ...
  currentTurn = computed<COLORS>(() => {
    if (this.currentMoveIndex() > 0) return this.chess.turn() as COLORS;
    return 'w';
  });

  currentTurnPcPlayerEnabled = computed(() => {
    return this.currentTurn() == 'w' ? this.whitePcPlayerEnabled() : this.blackPcPlayerEnabled();
  });
  ...
  private displayFenPositionEffect = effect(() => {
    if (this.gameFinished()) alert(this.gameFinished());
  ...
  private isPromotion(selectedPieceSquare: Square, destinationCell: Square) {
    if (
      !this.allowedDestinationCellsForSelectedPiece().find((s) =>
        s.coordinates.equals(destinationCell.coordinates),
      )
    )
      return false;

    if (selectedPieceSquare.piece()?.pieceSymbol == 'p') {
      if (destinationCell.coordinates.row == 1 || destinationCell.coordinates.row == 8) return true;
    }
    return false;
  }
  ...
  allowedDestinationCellsForSelectedPiece = computed(() => {
    const selectedPieceSquare = this.chessboardService.selectedSquare();
    if (selectedPieceSquare != null) {
      const selectedPieceCoordinates = selectedPieceSquare?.coordinates.toString();
      const allowedMoves = this.chess.moves({
        verbose: true,
        square: selectedPieceCoordinates as any,
      });
      const fromTO = allowedMoves.map((s) => s.to);
      const allowedCells = this.chessboardService.cells
        .flatMap((s) => s)
        .filter((s) => fromTO.join('').includes(s().coordinates.toString()))
        .map((s) => s());
      return allowedCells;
    }
    return [];
  ...
  movePiece(selectedPieceSquare: Square, destinationCell: Square) {
    ...
    this.chessboardService.selectedSquare.set(null);
  }
  ...
  move(
    ...
    if (this.chess.turn() == currentTurn) {
      ...
      let destinationMoves =
        typeof moveCoordinates === 'string'
      ...
      if (isValid) {
        if (destinationMoves.length == 1) {
          ...
          if (typeof moveCoordinates !== 'string') {
            const move = destinationMoves.find(
              (s) => s.promotion == (moveCoordinates as Move).promotion,
            );
